...
import BalanceBreakdown, { formatUGX as formatCurrency } from '../components/BalanceBreakdown';
...
type DashboardStatsV2 = DashboardStats & {
...
const StatCard = ({ label, value, sub, positive = true, gradient }: StatCardProps) => (
  ...
  >
    <Box
      sx={{
        position: 'absolute', top: -24, right: -24, width: 96, height: 96, borderRadius: '50%',
    ...
    <Typography
      sx={{
        fontSize: '0.75rem', fontWeight: 600, textTransform: 'uppercase', letterSpacing: 0.8, mb: 1,
    ...
    <Typography
      sx={{
        fontWeight: 800, fontSize: { xs: '1.5rem', sm: '1.75rem' }, lineHeight: 1.1, mb: 1,
        color: gradient ? '#fff' : tokens.color.textDark, wordBreak: 'break-word',
      }}
    >
      {value}
    </Typography>
    <Box sx={{ display: 'flex', alignItems: 'center', gap: 0.5 }}>
      ...
      <Typography
        sx={{
          ...
          color: gradient ? 'rgba(255,255,255,0.85)' : positive ? tokens.color.success : tokens.color.danger,
        }}
      >
        {sub}
      </Typography>
...
const SavingsDashboard = () => {
  ...
  const [balances, setBalances] = useState<BalanceModel | null>(null);
  const [balancesLoading, setBalancesLoading] = useState(false);

  const [collectorPeriod, setCollectorPeriod] = useState<'today' | 'cycle' | 'all_time'>('today');
  const [collectorSummaries, setCollectorSummaries] = useState<CollectorSummary[]>([]);
  const [collectorOverall, setCollectorOverall] = useState(ZERO_OVERALL);
  ...
  const fetchAllData = useCallback(async () => {
    ...
    try {
      ...
      setStats(data.stats as DashboardStatsV2);
  ...
  const fetchActiveCycleId = useCallback(async (): Promise<string | null> => {
    try {
  ...
  const fetchCollectorSummaries = useCallback(async (
    periodType: 'today' | 'cycle' | 'all_time',
    cycleId: string | null,
  ) => {
    try {
      ...
      if (periodType === 'today') {
        ...
        if (!cycleId) {
          ...
          setCollectorOverall(ZERO_OVERALL);
          return;
        }
        period = { type: 'cycle', cycleId };
      }
  ...
  useEffect(() => {
    (async () => {
      const cycleId = await fetchActiveCycleId();
      fetchAllData();
      fetchBalances(cycleId);
    })();
  }, [fetchActiveCycleId, fetchAllData, fetchBalances]);

  useEffect(() => {
    fetchCollectorSummaries(collectorPeriod, activeCycleId);
  }, [collectorPeriod, activeCycleId, fetchCollectorSummaries]);
  ...
  useEffect(() => {
    const refreshAll = async () => {
      ...
      fetchAllData();
      fetchBalances(cycleId);
      fetchCollectorSummaries(collectorPeriod, cycleId);
    };
  ...
  }, [collectorPeriod, fetchActiveCycleId, fetchAllData, fetchBalances, fetchCollectorSummaries]);
    ...
    return (
      <Box sx={{ p: { xs: 2, sm: 4 }, maxWidth: 640, mx: 'auto', background: tokens.color.bg, minHeight: '100vh' }}>
        ...
        <Button
          ...
          startIcon={<Refresh />}
          sx={{
            bgcolor: tokens.color.primary, color: '#fff', px: 3, py: 1, borderRadius: tokens.radius.md,
            fontWeight: 600, textTransform: 'none', '&:hover': { bgcolor: tokens.color.secondary }, boxShadow: 'none',
          }}
        >
          Retry
        </Button>
  ...
  const savingsThisMonth = stats.savings_this_month ?? balances?.monthDeposits ?? 0;
  const lifetimeBalance = stats.lifetime_savings ?? balances?.lifetimeBalance ?? stats.total_savings;
  const broughtForward = stats.brought_forward ?? balances?.broughtForward ?? 0;
  const monthNet = balances?.monthNet ?? lifetimeBalance - broughtForward;
    ...
    <Box sx={{ background: tokens.color.bg, minHeight: '100vh', width: '100%', overflowX: 'hidden', fontFamily: tokens.font.base, boxSizing: 'border-box' }}>
      ...
      >
        ...
        <Box sx={{ display: 'flex', alignItems: 'center', gap: 1 }}>
          <Button
            ...
            size="small"
            sx={{
              color: tokens.color.primary, textTransform: 'none', fontWeight: 600, fontSize: '0.78rem',
              borderRadius: tokens.radius.md, '&:hover': { bgcolor: tokens.color.primaryPale },
            }}
          >
            Refresh
          </Button>
          <IconButton sx={{ color: tokens.color.textMid }} aria-label="Notifications">
            <NotificationsNone />
          </IconButton>
        </Box>
      </Box>
      ...
      <Box sx={{ px: { xs: 2, sm: 3, md: 4 }, py: 3, maxWidth: '100%', boxSizing: 'border-box', overflowX: 'hidden' }}>
        ...
        >
          ...
          <Chip label="Current Month" size="small" sx={{ bgcolor: 'rgba(255,255,255,0.2)', color: '#fff', fontWeight: 600, fontSize: '0.72rem', mb: 1.5 }} />
          ...
          <Typography sx={{ fontWeight: 800, fontSize: { xs: '2rem', sm: '2.6rem' }, lineHeight: 1.05, mb: 0.5 }}>
            {balancesLoading && !stats.savings_this_month
              ? <CircularProgress size={28} sx={{ color: '#fff' }} />
              : formatCurrency(savingsThisMonth)}
          </Typography>
          <Typography sx={{ fontSize: '0.8rem', opacity: 0.85, mb: 0.75 }}>
            Deposits received this month
            {balances ? ` · ${formatCurrency(balances.monthWithdrawn)} withdrawn` : ''}
          </Typography>
        ...
        <Box sx={{ mb: 3 }}>
          <Typography sx={{ fontWeight: 700, fontSize: '0.95rem', color: tokens.color.textDark, mb: 1.25 }}>
            SACCO balance
          </Typography>
          <BalanceBreakdown
            broughtForward={broughtForward}
            thisMonth={monthNet}
            total={lifetimeBalance}
        ...
        <Box
          sx={{
            ...
            gap: { xs: 2, sm: 2.5 }, mb: 3,
          }}
        >
          <StatCard
            label="Total Savings"
            value={formatCurrency(lifetimeBalance)}
            sub="Lifetime, all months"
            positive
          ...
          <StatCard
            ...
            sub="Monthly fees collected this month"
            positive
          />
          <StatCard
        ...
        <Box sx={{ display: 'grid', gridTemplateColumns: { xs: '1fr', lg: 'repeat(2, 1fr)' }, gap: { xs: 2, sm: 2.5 }, mb: 3 }}>
            ...
            {savingsTrendData.length > 0 ? (
              <ResponsiveContainer width="100%" height={280}>
                <AreaChart data={savingsTrendData} margin={{ top: 4, right: 4, left: 0, bottom: 0 }}>
                  ...
                  <XAxis dataKey="month" stroke={tokens.color.textMuted} tick={{ fontSize: 11, fill: tokens.color.textMuted }} axisLine={false} tickLine={false} />
                  <YAxis stroke={tokens.color.textMuted} tick={{ fontSize: 10, fill: tokens.color.textMuted }} axisLine={false} tickLine={false} width={60} tickFormatter={(v) => `${(v / 1000).toFixed(0)}k`} />
                  <Tooltip content={<ChartTooltip />} />
                  <Legend wrapperStyle={{ fontSize: '0.78rem', color: tokens.color.textMid, paddingTop: 12 }} />
                  <Area type="monotone" dataKey="savings" stroke={tokens.color.primary} strokeWidth={2.5} fill="url(#gSavings)" name="Savings (month-end balance)" />
                  <Area type="monotone" dataKey="profit" stroke={tokens.color.primaryLight} strokeWidth={2.5} fill="url(#gProfit)" name="Monthly fees" />
                </AreaChart>
              </ResponsiveContainer>
            ) : (
            ...
            {weeklyDeposits.length > 0 ? (
              <ResponsiveContainer width="100%" height={280}>
                <BarChart data={weeklyDeposits} margin={{ top: 4, right: 4, left: 0, bottom: 0 }}>
                  <XAxis dataKey="day" stroke={tokens.color.textMuted} tick={{ fontSize: 11, fill: tokens.color.textMuted }} axisLine={false} tickLine={false} />
                  <YAxis stroke={tokens.color.textMuted} tick={{ fontSize: 10, fill: tokens.color.textMuted }} axisLine={false} tickLine={false} width={60} tickFormatter={(v) => `${(v / 1000).toFixed(0)}k`} />
                  <Tooltip content={<ChartTooltip />} />
                  <Bar dataKey="amount" fill={tokens.color.primaryLight} radius={[8, 8, 0, 0]} name="Deposits" maxBarSize={48} />
                </BarChart>
              </ResponsiveContainer>
            ) : (
              <Box sx={{ textAlign: 'center', py: 6 }}>
                ...
                <Table sx={{ tableLayout: 'fixed', width: '100%' }} size="small">
                  <TableHead>
                    <TableRow sx={{ background: tokens.color.surfaceAlt }}>
                      {[
                        { label: 'Member', width: '50%' },
                        { label: 'Amount', width: '30%' },
                        { label: 'Date', width: '20%', hideXs: true },
                      ].map((h) => (
                        <TableCell
                          key={h.label}
                          sx={{
                            width: h.width, fontWeight: 700, fontSize: '0.72rem', textTransform: 'uppercase',
                            letterSpacing: 0.5, color: tokens.color.textMuted,
                            borderBottom: `2px solid ${tokens.color.border}`, py: 1.25,
                            display: h.hideXs ? { xs: 'none', sm: 'table-cell' } : undefined,
                          }}
                        >
                          {h.label}
                        </TableCell>
                    ...
                    {recentTransactions.map((tx, idx) => (
                      <TableRow
                        ...
                        sx={{
                          ...
                          '&:hover': { background: tokens.color.primaryPale }, transition: 'background 0.15s',
                        }}
                      >
                        <TableCell sx={{ py: 1.25, overflow: 'hidden' }}>
                          <Box sx={{ display: 'flex', alignItems: 'center', gap: 1.25, minWidth: 0 }}>
                            <Avatar sx={{ bgcolor: avatarColor(idx), width: 30, height: 30, fontSize: '0.7rem', fontWeight: 700, flexShrink: 0 }}>
                              {getInitials(`${tx.member.first_name} ${tx.member.last_name}`)}
                            </Avatar>
                            <Typography sx={{ fontSize: '0.82rem', fontWeight: 600, color: tokens.color.textDark, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>
                              {tx.member.first_name} {tx.member.last_name}
                            </Typography>
                          </Box>
                        </TableCell>
                        <TableCell sx={{ py: 1.25 }}>
                          <Typography sx={{ fontSize: '0.82rem', fontWeight: 700, color: tokens.color.success, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>
                            {formatCurrency(tx.amount)}
                          </Typography>
                        </TableCell>
                        <TableCell sx={{ py: 1.25, fontSize: '0.78rem', color: tokens.color.textMuted, display: { xs: 'none', sm: 'table-cell' }, whiteSpace: 'nowrap' }}>
                          {formatDate(tx.date)}
                        </TableCell>
                      </TableRow>
                    ))}
              ...
              <Box>
                {topSavers.map((saver, idx) => (
                  <Box key={idx}>
                    <Box
                      sx={{
                        display: 'flex', justifyContent: 'space-between', alignItems: 'center', py: 1.75, px: 0.5,
                        borderRadius: tokens.radius.md, '&:hover': { background: tokens.color.primaryPale }, transition: 'background 0.15s',
                      }}
                    >
                      <Box sx={{ display: 'flex', alignItems: 'center', gap: 1.5, minWidth: 0 }}>
                        <Box
                          sx={{
                            ...
                            bgcolor: idx === 0 ? tokens.color.gold : idx === 1 ? tokens.color.silver : idx === 2 ? tokens.color.bronze : tokens.color.primaryPale,
                            display: 'flex', alignItems: 'center', justifyContent: 'center',
                            fontWeight: 800, fontSize: '0.85rem', color: idx < 3 ? '#fff' : tokens.color.primary, flexShrink: 0,
                          }}
                        >
                          {idx + 1}
                        </Box>
                      ...
                      <Typography sx={{ fontWeight: 700, fontSize: '0.875rem', color: tokens.color.primary, flexShrink: 0, ml: 1 }}>
                        {formatCurrency(saver.total_savings)}
                      </Typography>
                    </Box>
                    {idx < topSavers.length - 1 && <Divider sx={{ borderColor: tokens.color.border }} />}
                  </Box>
                ))}
              </Box>
            ) : (
          ...
          <Box sx={{ display: 'flex', gap: 1, mb: 2.5, flexWrap: 'wrap' }}>
            {([
              ...
              <Button
                ...
                size="small"
                sx={{
                  textTransform: 'none', fontWeight: 700, fontSize: '0.8rem', borderRadius: tokens.radius.md, px: 2,
                  ...
                  '&:hover': { bgcolor: collectorPeriod === opt.key ? tokens.color.secondary : tokens.color.primaryPale },
                }}
              >
                {opt.label}
              </Button>
            ...
            <>
              <Box sx={{ display: 'grid', gridTemplateColumns: { xs: '1fr 1fr', sm: 'repeat(3, 1fr)' }, gap: 1.5, mb: 3 }}>
                {[
                  ...
                  <Box key={stat.label} sx={{ p: 1.75, borderRadius: tokens.radius.md, bgcolor: tokens.color.surfaceAlt, border: `1px solid ${tokens.color.border}` }}>
              ...
              {collectorSummaries.length === 0 ? (
                ...
                <TableContainer sx={{ overflowX: 'auto', width: '100%' }}>
                  <Table sx={{ minWidth: 560 }} size="small">
                    <TableHead>
                      <TableRow sx={{ background: tokens.color.surfaceAlt }}>
                        {['Collector', 'Collected', 'Withdrawn', 'Profit', 'Members', 'Entries'].map((h) => (
                          <TableCell
                            key={h}
                            align={h === 'Collector' ? 'left' : 'right'}
                            sx={{ fontWeight: 700, fontSize: '0.72rem', textTransform: 'uppercase', letterSpacing: 0.5, color: tokens.color.textMuted, borderBottom: `2px solid ${tokens.color.border}`, py: 1.25 }}
                          >
                            {h}
                          </TableCell>
                        ))}
                    ...
                    <TableBody>
                      {collectorSummaries.map((c, idx) => (
                        <TableRow key={c.collector.id} sx={{ background: idx % 2 === 0 ? tokens.color.surface : tokens.color.surfaceAlt, '&:hover': { background: tokens.color.primaryPale } }}>
                          <TableCell sx={{ py: 1.25 }}>
                          ...
                          <TableCell align="right" sx={{ py: 1.25 }}>
                            <Typography sx={{ fontSize: '0.82rem', fontWeight: 700, color: tokens.color.success, whiteSpace: 'nowrap' }}>{formatCurrency(c.total_saved)}</Typography>
                          </TableCell>
                          <TableCell align="right" sx={{ py: 1.25 }}>
                            <Typography sx={{ fontSize: '0.82rem', fontWeight: 700, color: tokens.color.danger, whiteSpace: 'nowrap' }}>{formatCurrency(c.total_withdrawn)}</Typography>
                          </TableCell>
                          <TableCell align="right" sx={{ py: 1.25 }}>
                            <Typography sx={{ fontSize: '0.82rem', fontWeight: 700, color: tokens.color.primary, whiteSpace: 'nowrap' }}>{formatCurrency(c.total_profit)}</Typography>
                          </TableCell>
                          <TableCell align="right" sx={{ py: 1.25, fontSize: '0.8rem', color: tokens.color.textMid }}>{c.members_count}</TableCell>
                          <TableCell align="right" sx={{ py: 1.25, fontSize: '0.8rem', color: tokens.color.textMid }}>{c.entries_count}</TableCell>
                        </TableRow>
                      ))}
                    </TableBody>
                  </Table>
